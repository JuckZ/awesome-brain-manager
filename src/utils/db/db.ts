...
export const getAllTables = async (sqlJS: SqlJsStatic, path: string): Promise<string[]> => {
    ...
    return res.reduce(
        (p, c, i) => [
            ...p,
            {
                cols: c.columns,
                rows: c
                    ? c.values.map(r => c.columns.reduce((prev, curr, index) => ({ ...prev, [curr]: r[index] }), {}))
                    : [],
            },
        ],
        [],
...
export const updateDBConditionally = (db: Database, tables: DBTables, condition: string) => {
